#!/bin/bash
# v16-builder.sh - Costruzione da zero (con /bin/init corretto)
echo "v16-builder.sh - Costruzione da zero, con /bin/init corretto..."
...
STAGING=./staging_v16                           # Directory di lavoro pulita
...
# ==================================================================
echo "EGGS: Inietto manually i file di live-boot..."
...
# ==================================================================
# 4. FASE INIEZIONE MANUALE (FILE ESSENZIALI e /bin/init CORRETTO)
# ==================================================================
...
# === IL FIX (v16) ===
# Copiamo busybox in /bin/sh (fallback) e creiamo /bin/init (primario)
cp "${CHROOT_PATH}/bin/busybox" ./bin/sh

# Copiamo i file di UDEV dai percorsi REALI
...
# Crea l'entrypoint 'init' nel percorso che il kernel cerca
cat > ./bin/init << EOF
#!/bin/sh

echo "EGGS: init (v16) partito."
