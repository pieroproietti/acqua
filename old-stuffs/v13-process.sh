#!/bin/bash
# v13-builder.sh - Costruzione da zero (con init script corretto)
echo "v13-builder.sh - Costruzione da zero, con init script corretto..."
...
STAGING=./staging_v13                           # Directory di lavoro pulita
...
# Copiamo i file per UDEV (gestore /dev)
mkdir -p ./lib/udev
cp -R "${CHROOT_PATH}/lib/udev/rules.d" ./lib/udev/
cp "${CHROOT_PATH}/sbin/udevd" ./sbin/
cp "${CHROOT_PATH}/sbin/udevadm" ./sbin/
